...
annotateWithCate<-function(myGRAndAttributes, thresholdsForgrLScores, thresholdsForgrLDistance){
  ...
  for (jGR in 1:length(thresholdsForgrLScores)){
    ...
    if (median(thresholdValues) > 1e3){
      cateNames <- analysePeaks::cateNamesFromThreshold(thresholdValues = thresholdValues/1e3,
                                                        lastCateName = paste0("no", nameJ),
                                                        unit = "k")
    } else {
      cateNames <- analysePeaks::cateNamesFromThreshold(thresholdValues = thresholdValues,
                                                        lastCateName = paste0("no", nameJ),
                                                        unit = "")
    }
    whatString = paste("score of", nameJ)
    ...
    allCates[[length(allCates) + 1]] <- list(nameOfOriCol = paste0("best", nameJ, "Score"),
  ...
  for (jGR in 1:length(thresholdsForgrLDistance)){
    ...
    whatString = paste("distance to the closest", nameJ)
    allCates[[length(allCates) + 1]] <- list(nameOfOriCol = paste0("distanceToNearest", nameJ),
